...
连续兴趣故事和用户“编故事”共用分阶段生成流水线：`排队` → `生成多套候选季纲` → `总编选优` → `逐集双首稿` → `加权择优` → `多维质量评审` → `编辑润色` → `结构与词汇门禁` → `独立语义终审` → `逐集入库`。每集都有独立任务、新信息、不可逆变化和禁止重复项；后续集还必须达到第一集建立的质量基线。客户端会同步显示当前步骤、具体集数和总进度；第一集通过质量门禁后立即上架，用户可以先读，其余章节继续在后台生成。
