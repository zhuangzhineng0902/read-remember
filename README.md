...
连续兴趣故事和用户“编故事”共用分阶段生成流水线：`排队` → `生成多套候选季纲` → `总编选优` → `逐集五份 M3 直接首稿` → `用统一四维规则并行评分` → `达到 7/7.5 的优秀原稿直接通过，否则锁定最高分稿为主骨架并融合` → `必要时增益优化` → `最终结构与词汇修稿` → `语义防退化确认` → `单独生成题目` → `逐集入库`。每集先做容量检查，把硬叙事工作压成与四段一一对应的四项任务、最多两个线索动作；一至两句必要的上集状态承接不算重复，只有把旧事实重新当成新发现或重新表演已完成动作才会扣连续性分。高分原稿如果只是超长，会从原稿做最多两次保守压缩并重新核对核心事件，不再混入其他候选导致剧情退化。未过线融合稿中得分最高的一篇会作为精英候选进入下一轮，并与 5 篇全新初稿重新比较；失败经验最多保存 20 条、按类别压缩为最多 8 条，并对本任务后续全部章节生效。第一集和后续集由同一个独立评审器、同一提示和同一参数评分；旧检查点的第一集只会在评分规则版本变化时重新校准一次。剧情定稿前不再反复做结构修稿；多维评审始终覆盖剧情、儿童吸引力、分级语言和连续性。客户端会同步显示当前步骤、具体集数和总进度；第一集通过质量门禁后立即上架，用户可以先读，其余章节继续在后台生成。
