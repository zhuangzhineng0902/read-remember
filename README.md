...
连续兴趣故事和用户“编故事”共用分阶段生成流水线：`排队` → `生成多套候选季纲` → `总编选优` → `逐集三份首稿` → `一次批量多维评审并择优` → `复用候选评审` → `必要时剧情重写` → `最终结构与词汇修稿` → `语义防退化确认` → `单独生成题目` → `逐集入库`。剧情定稿前不再反复做结构修稿；多维评审始终覆盖剧情、儿童吸引力、分级语言和连续性。每集都有独立任务、新信息、不可逆变化和禁止重复项，后续集还必须达到第一集建立的质量基线。客户端会同步显示当前步骤、具体集数和总进度；第一集通过质量门禁后立即上架，用户可以先读，其余章节继续在后台生成。
