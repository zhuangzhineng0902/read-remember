...
连续兴趣故事和用户“编故事”共用分阶段生成流水线：`排队` → `生成多套候选季纲` → `总编选优` → `逐集五份 M3 直接首稿` → `用统一四维规则并行评分` → `达到 7/7.5 的优秀原稿直接通过，否则锁定最高分稿为主骨架并融合` → `必要时增益优化` → `最终结构与词汇修稿` → `语义防退化确认` → `单独生成题目` → `逐集入库`。每集先做容量检查，把硬叙事工作压成与四段一一对应的四项任务、最多两个线索动作；每张段落卡同时指定句子数与单句词数上限，并按文章上限动态压低模型输出 Token，尽量让首稿直接落入长度范围。候选执行只使用上一集摘要和结构化状态，不再重复输入完整正文。一至两句必要的上集状态承接不算重复，只有把旧事实重新当成新发现或重新表演已完成动作才会扣连续性分。高分原稿如果只是超长，会从当前唯一稿件做最多两次保守压缩并重新核对核心事件；长度内容纠错与 JSON 结构纠错使用不同通道，后一次不会同时携带原长稿和压缩稿。压缩结果还要通过逐段词汇重合防改写检查，不再混入其他候选导致剧情退化。每次队列尝试现在只允许一批 5 份新候选和一份融合稿，不再在一次可见重试内隐藏第二批候选或第二次融合；未过线融合稿中得分最高的一篇仍会写入检查点，在下一次有明确额度的自动重试中作为精英候选。失败经验最多保存 20 条、按类别压缩为最多 8 条，并对本任务后续全部章节生效。第一集和后续集由同一个独立评审器、同一提示和同一参数评分；旧检查点的第一集只会在评分规则版本变化时重新校准一次。客户端会同步显示当前步骤、具体集数和总进度；第一集通过质量门禁后立即上架，用户可以先读，其余章节继续在后台生成。

最终季纲完成后会保存检查点；每集的候选初稿、编辑稿、结构修稿、语义评审和剧情修稿也会分阶段保存。模型超时、返回异常或服务重启时，重试会从本集最近的具体阶段继续。每集全文重写统一最多两次，结构与语义修稿不会互相嵌套；低于编辑底线的候选会重新生成。详细阶段、流式请求和恢复行为见 [服务端 README](./server/README.md#故事生成阶段与断点续跑)。
