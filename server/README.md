...
全部九个内置栏目都能生成连续故事：`military`、`art`、`science`、`why`、`fantasy`、`mecha`、`cultivation`、`tiger` 和 `cat`。默认并行生成 3 套候选季纲，再由总编模型融合选优；每一集并行生成 3 份首稿，然后用 1 次批量评审分别检查剧情、儿童吸引力、分级语言和连续性并加权择优。最终策划包含故事圣经、角色成长、固定称呼、场景因果链、线索账本和每集独立任务合同。脚本还会读取同栏目已有的选择、完成、阅读进度、答题、生词与下一集续读等聚合反馈，用于调整新一季的钩子、节奏和难度。只有篇幅、句长、真实词频覆盖率、目标词、团队协作、题目结构及四维语义质量都达到质量线的章节才会写入数据库。

每集任务合同包含本集不可替代的叙事任务、必须带来的新信息、不可逆状态变化和不得重复的前集内容。后续章节同时接收上一集最终正文与结构化状态；系统检测近似重复句。独立终审要求四个维度均不低于 8 分，并且后续集的各维度不得比第一集低超过 1 分、平均分不得低超过 0.5 分。流水线先复用候选稿评审判断剧情，必要时只做一次剧情重写；剧情通过后再统一执行最终结构与词汇修稿，最后确认修稿没有造成语义退化，避免剧情重写前后各做一遍结构修稿。
...
短句比例检查只统计叙述句，`"Wait!"`、`'Look!'` 等不超过 4 个词的带引号短对话不会被误判为碎片句。模型网络请求与 JSON 结构纠错使用独立预算：普通请求默认最多等待 120 秒，完整正文重写单独允许 480 秒且每次只进行 1 次网络尝试。请求使用流式响应，并通过 Undici dispatcher 把响应头和响应体时限设置为业务超时再加 30 秒，避免 480 秒重写被 HTTP 客户端默认的 300 秒响应头时限提前截断。季纲、正文、批量评审和题目分别使用 10240、8192、8192 和 2048 的输出 Token 上限；模型在 Token 边界结束时会先解析已经收到的完整内容。若流式响应只有推理而没有最终正文，不会再当成网络问题重复相同请求，而会记录模型、思考模式、Token 上限、finish reason、事件数、推理字符和业务状态。网络错误仍由同一模型按短预算重试；空内容、JSON 无法解析或字段结构错误进入下一次结构尝试时，统一交给 `structureRepairModel`。当前配置形成 `MiniMax-M2.7 → MiniMax-M3 无思考` 或 `MiniMax-M3 深度思考 → MiniMax-M3 无思考` 的纠错路径，不再把结构修复切给无法稳定关闭思考的 M2.7。模型策略按任务性质分流：候选策划、创意初稿和多维评审允许思考；剧情定向重写、最终结构修稿、阅读题和缺失元数据补齐从第一次请求起就使用无思考模式，让模型直接交付目标 JSON，而不是把输出额度消耗在长推理上。可通过 `timeoutMs`、`rewriteTimeoutMs`、`networkRetries`、`structureRetries` 和 `structureRepairModel` 调整时间、重试及纠错模型。
...
| `repairing` | 先剧情重写，再做最终结构修稿 | 每集完整重写总共最多两次，结构修稿只在剧情定稿后执行一次 |
...
最终季纲生成后会立即保存检查点；每集选出候选初稿后，以及形成编辑稿、结构词汇修稿、语义评审和剧情修稿后也都会保存进行中章节。失败任务重试时会从具体阶段继续，而不是从本集开头重跑。候选稿四维均分低于 7 分时只重新生成一份全新候选，不对明显不合格的初稿连续润色。章节通过全部门禁后会立即入库；第一集同时绑定到用户书架并解锁，客户端最多约 5 秒即可轮询到，用户阅读时剩余章节仍在后台生成。
...
模型返回先经过容错解析，再进入 Zod 结构校验。解析层会处理 Markdown 代码围栏、JSON 前后说明文字、多个 JSON 片段、数组包裹、常见字段信封和可修复的 JSON 语法；结构仍不符合要求时，会把具体字段错误发回模型要求重新输出。普通请求默认 120 秒、2 次网络尝试和 2 次结构尝试；完整正文重写默认 480 秒、1 次网络尝试和 1 次结构尝试，并计入每集最多两次的统一重写预算。若完整重写已经返回合法的 `title` 和 `paragraphs`，只是漏掉 `targetWords`、连续性状态或质量证据，系统会保留正文并用独立短请求只补这些元数据，不再重新生成长正文，也不额外占用完整重写额度。
