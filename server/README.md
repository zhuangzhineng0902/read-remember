...
flowchart TD
  ...
  E --> V[词汇前检]
  V --> S[批量筛选或单稿评审]
  S --> L{Starter 剧情成立但语言未过线?}
  L -- 是 --> LE[锁定事件顺序做一次语言编辑]
  LE -- 仍失败 --> LP[下次续跑简化未发布行动]
  LP --> E
  L -- 否 --> F[独立终审或一次有界融合]
  LE -- 通过 --> M0
  F --> FV{最终词汇过线?}
  FV -- 是 --> M0[保存 metadata_pending]
  FV -- 否 --> VE[保留语义合格稿并定向换词一次]
  VE --> VR[重新执行语义终审]
  VR -- 通过 --> M0
  VR -- 失败 --> LP
  M0 --> M[生成状态和原文证据]
...
- 正文一旦通过语义终审，最终词汇失败不再返回候选生成；程序保留该稿、定向换词一次，并在采用前重新执行语义终审。
- 元数据后的局部修复若耗尽词汇额度，只要 Starter 正文仍保持语义合格，就保存该正文并转入一次未发布行动减载，不把它丢弃为新候选。
- Starter 稿若剧情、吸引力和连续性均成立，但分级语言成为瓶颈，锁定该稿的四段事件顺序，只做一次时态、搭配、指代和词汇编辑，不再进入同轮融合。
- 融合稿若只剩 Starter 语言维度未达标，同样直接进入锁稿语言编辑，不绕回新候选。
- 语言编辑与定向换词分别记录为 `starter_language_load` 和 `starter_lexical_load`，不能互相消耗额度或混用报错。
- 检查点按集独立保存语言编辑、词汇修复和专项行动简化状态，并绑定正文 SHA-256、季纲 SHA-256 与换词前后覆盖率。普通连续性季纲校正不等于专项行动简化。
- 语言或词汇负担触发专项行动简化后，程序立即自动重试本集；只有简化后的新稿仍失败，才返回“需要修改后继续”，并准确报告语义分数、覆盖率和待修改项。
...
恢复时会核对正文 `textHash`。词汇只允许一次正文局部修改；元数据允许两次独立重建，使简化后的正文在首次证据生成失败时能够自动再试一次。调用额度都在网络请求前持久化，且不会增加 `fullRewriteCount`。旧检查点保留已经消耗的完整重写次数，已标记 `lexicalRepairExhausted` 的稿件不会重新获得换词额度。

检查点字段缺失、层级错误或 JSON 损坏时，任务会明确失败并记录解析原因；即使已有发布章节，也绝不会把损坏检查点当成“没有检查点”并从第一集重建。手动重试同样返回 `CUSTOM_STORY_REQUIRES_REVISION`，直到检查点被修复。

### 系列版本与幂等发布

- `series_key` 是不可随标题变化的故事版本 ID；定制故事使用请求 ID，章节身份是“版本 ID + 集号”。
- `idx_articles_story_version_episode` 为该身份提供数据库唯一约束。标题变化或同一检查点重试会更新原文章，不会发布另一篇第一集。
- 重新创作应创建新的定制故事请求，因此获得新的版本 ID；普通重试必须沿用原版本。
- 升级旧数据库时，若同一请求键下存在多个系列标题，当前请求标题保留原版本键，其余文章按标题迁移到确定性的 `:legacy:` 版本键；文章 ID、答题、阅读进度和投递记录不变，当前请求的 `article_ids_json` 只保留当前版本。
...
| P0-6 系列版本隔离 | 已完成 | 第三集失败不重建第一集；改名按版本 ID 幂等更新；旧新系列迁移后不串版 |
| P0-7 检查点损坏熔断 | 已完成 | 非法 JSON/Schema 明确失败；启动恢复和手动重试均不得静默重开 |
| P0-8 修复状态独立 | 已完成 | 语言、词汇、行动简化分别持久化；普通季纲校正不消耗专项额度，报错包含真实分数与覆盖率 |
