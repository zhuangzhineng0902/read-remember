...
每集任务合同包含本集不可替代的叙事任务、必须带来的新信息、不可逆状态变化和不得重复的前集内容。容量检查会把这些内容合并为与四段一一对应的四项硬任务，并把每集强制线索动作限制为最多两个；其余设定只在有篇幅时选用。后续章节同时接收上一集最终正文与结构化状态；一至两句必要的当前状态承接不会被当作重复，只有把旧事实重写成新发现、主要冲突，或重新表演已完成动作才会扣连续性分。第一集与后续集使用同一评审提示、模型参数和评分标尺；恢复旧检查点时，第一集只在评分规则版本变化后重新校准一次并写回检查点，避免每次重启重复评审。融合稿先经过段数、词数和纯英文硬门禁，再接受独立终审；剧情、儿童吸引力、分级英语和连续性四项必须至少 7 分，四维均分至少 7.5。未达标稿不会生成元数据或进入后续润色；其中最高分融合稿会连同评审一起写入检查点，下一轮作为第 6 份精英候选与 5 篇全新初稿重新比较，而不是盲目作废。若原始候选已通过语义严选但仅长度超限，系统会从该原稿做定长压缩，再用独立四维评审复核四项核心事件、连续状态与线索；第一次压缩损伤叙事时只允许再做一次不引入其他候选事件的保守压缩，不再进入融合。普通融合仍以最高分稿作为编辑底稿，其他稿件经蓝图合计最多贡献两个局部增强。每次融合后先与编辑底稿比较，只有均分上升才成为下一次底稿，退化稿立即丢弃。同一批素材最多融合两次，每个运行最多两批素材；每一集独立拥有最多三次质量失败自动续跑额度，章节上架后下一集从零计数，次数持久化到数据库。恢复断点时，已完成章节只幂等补齐文章记录，不再次触发发布回调，因此不会重置当前集的重试次数或形成无限循环。最终失败信息会包含原稿词数范围、前置淘汰数、独立评分次数和本轮最佳四维分数。严选原稿通过后，剧情润色采用“先直接生成重构蓝图 JSON、再独立执行正文 JSON”；只有润色稿仍满足门禁且四维均分高于原稿时才采用，否则保留严选原稿。
