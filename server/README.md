...
| `quality_check` | 编辑稿已完成，正在自动质量检查 | 检查篇幅、句长、词频覆盖、目标词、协作主题和题目结构 |
| `repairing` | 正在进行第 1/3 次定向修稿 | 只针对自动检测到的问题修订，并重新检查 |
| `saving` | 正在保存文章并解锁第一集 | 将全部章节写入题库和用户故事书架 |
| `completed` | 故事已完成，可以开始阅读 | 第一集已解锁，后续章节按答题完成情况逐集解锁 |
| `failed` | 已保存前 2 集，重试后将从第 3 集继续 | 显示错误和可恢复位置，等待用户重试 |

最终季纲生成后会立即保存检查点；此后每一集通过质量门禁，就把该集成稿、连续性状态和质量结果一并保存。服务重启、模型超时或 JSON 返回异常不会清除已经完成的工作：重新启动服务会自动恢复排队中的任务，失败任务点击重试会从最新检查点继续，不会重复消耗已完成章节的模型调用。
