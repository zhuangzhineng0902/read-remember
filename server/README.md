...
模型返回先经过容错解析，再进入 Zod 结构校验。解析层会处理 Markdown 代码围栏、JSON 前后说明文字、多个 JSON 片段、数组包裹、常见字段信封和可修复的 JSON 语法。MiniMax M2.7/M3 的 OpenAI 兼容接口不能依赖原生 `response_format.json_schema` 约束，因此可确定修复的展示差异优先在本地归一化：嵌套段落数组会拼合；完整正文无论意外返回 1-2 段、3 段或 5 段以上，都会按原句顺序和词数均衡本地归并为四段且不增删英文词元；评审数组过长会本地截断。若正文已包含 `title` 和可恢复的 `paragraphs`，系统会锁定正文并用独立短请求补 `targetWords`、连续性状态与质量证据。只有标题或正文内容真正缺失时，才把具体字段错误交给 `structureRepairModel` 重写，避免为包装格式和段数反复生成长正文。
