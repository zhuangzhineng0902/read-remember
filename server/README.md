...
全部九个内置栏目都能生成连续故事：`military`、`art`、`science`、`why`、`fantasy`、`mecha`、`cultivation`、`tiger` 和 `cat`。默认并行生成 3 套候选季纲，再由总编模型融合选优；每一集也会并行生成 3 份思考初稿，但不直接选其中一篇。总编先用思考模式提炼各稿最强的钩子、因果、伙伴互动、感官细节、自然英语和悬念，形成一条统一因果骨架，再用无思考模式写成一篇全新的融合稿。最终策划包含故事圣经、角色成长、固定称呼、场景因果链、线索账本和每集独立任务合同。脚本还会读取同栏目已有的选择、完成、阅读进度、答题、生词与下一集续读等聚合反馈，用于调整新一季的钩子、节奏和难度。只有篇幅、句长、真实词频覆盖率、目标词、团队协作、题目结构及四维语义质量都达到质量线的章节才会写入数据库。

每集任务合同包含本集不可替代的叙事任务、必须带来的新信息、不可逆状态变化和不得重复的前集内容。后续章节同时接收上一集最终正文与结构化状态；系统检测近似重复句。融合稿先经过段数、词数和纯英文硬门禁，再接受一次独立绝对质量终审；剧情、儿童吸引力、分级英语和连续性四项必须全部达到 8 分。未达标稿不会生成元数据或进入后续润色：同一批素材最多重新融合一次，仍不达标才换一批全新初稿。严选原稿通过后，剧情润色采用“思考生成重构蓝图、无思考执行完整 JSON”；只有润色稿仍满足四项 8 分且四维均分高于原稿时才采用，否则保留严选原稿。
...
短句比例检查只统计叙述句，`"Wait!"`、`'Look!'` 等不超过 4 个词的带引号短对话不会被误判为碎片句。模型网络请求与 JSON 结构纠错使用独立预算：普通请求默认最多等待 120 秒，完整正文重写单独允许 480 秒且每次只进行 1 次网络尝试。请求使用流式响应，并通过 Undici dispatcher 把响应头和响应体时限设置为业务超时再加 30 秒，避免 480 秒重写被 HTTP 客户端默认的 300 秒响应头时限提前截断。季纲、正文、评审和题目分别使用独立输出 Token 预算；模型在 Token 边界结束时会先解析已经收到的完整内容。若流式响应只有推理而没有最终正文，不会再当成网络问题重复相同请求，而会记录模型、思考模式、Token 上限、finish reason、事件数、推理字符和业务状态。网络错误仍由同一模型按短预算重试；空内容、JSON 无法解析或字段结构错误进入下一次结构尝试时，统一交给 `structureRepairModel`。候选初稿、融合蓝图和剧情重构蓝图开启思考；融合正文、润色正文、独立评审、最终结构修稿、阅读题和缺失元数据补齐关闭思考，避免长推理挤占最终 JSON。可通过 `timeoutMs`、`rewriteTimeoutMs`、`networkRetries`、`structureRetries` 和 `structureRepairModel` 调整时间、重试及纠错模型。
...
最终季纲生成后会立即保存检查点；融合稿通过四项 8 分严选并补齐元数据后，以及形成结构词汇修稿、语义评审和剧情润色后也都会保存进行中章节。失败任务重试时会从具体阶段继续，而不是重跑已完成章节。未达到 8 分的融合稿不会流入元数据、润色或命题环节；系统先携带评审经验重新融合，同批仍失败才生成新素材。章节通过全部门禁后会立即入库；第一集同时绑定到用户书架并解锁，客户端最多约 5 秒即可轮询到，用户阅读时剩余章节仍在后台生成。
