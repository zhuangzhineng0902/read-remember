...
flowchart TD
  ...
  LE -- 通过 --> M0
  F --> FV{覆盖率偏低?}
  FV -- 否 --> M0[保存 metadata_pending]
  FV -- 是 --> VE[保留语义合格稿并保守简化一次]
  VE --> VR[重新执行语义终审]
  VR -- 通过 --> M0
  VR -- 失败 --> M0
  M0 --> M[生成状态和原文证据]
...
- 高频词覆盖率保留为编辑诊断和学习词选择依据，不作为发布硬门禁；`pup`、`telescope` 等题材核心词可以保留为目标词。
...
- 所有满足正文完整性与宽松篇幅上限的候选都可进入语义评审；覆盖率只记录难词诊断，不再提前淘汰候选。
- 语义终审通过后，覆盖率偏低最多触发一次保守简化；只有简化稿仍通过语义终审才采用，否则保留原稿。
- 一次简化后无论覆盖率提高多少都继续后续流程，不再因覆盖率执行全文换词、扩写、压缩、季纲减载或候选重生。
- Starter 稿若剧情、吸引力和连续性均成立，但分级语言成为瓶颈，锁定该稿的四段事件顺序，只做一次时态、搭配、指代和词汇编辑，不再进入同轮融合。
- 融合稿若只剩 Starter 语言维度未达标，同样直接进入锁稿语言编辑，不绕回新候选。
- Starter 自然英语确实未达标时仍可记录 `starter_language_load` 并执行一次锁稿语言编辑；旧 `starter_lexical_load` 仅作检查点兼容，不再阻断重试。
...
恢复时会核对正文 `textHash`。覆盖率偏低最多允许一次正文局部简化，但从不阻断发布；元数据允许两次独立重建。调用额度都在网络请求前持久化，且不会增加 `fullRewriteCount`。旧检查点中的 `lexicalRepairExhausted` 与 `starter_lexical_load` 仅兼容读取，不再触发词汇失败循环。
...
| P0-2 结构化故障路由 | 已完成 | 词汇覆盖率只诊断；元数据、语言与正文错误独立修复，局部额度跨重启保持 |
...
| P0-8 修复状态独立 | 已完成 | 自然英语修复与覆盖率诊断分离；低覆盖率不再消耗全文重写或季纲简化额度 |
